...
- **Farm tab totals** — shows total crystals/min across all farming droids, and a plain-language summary of whatever buff is currently active (including Nebulfox's active/cooldown/ready state).
- **Small polish**: variant badges now show on Farm slot cards, Inventory shows crystal balance alongside Paint/Nova Chips, wishlist entries show player ID next to username, and Dex entries for evolution-only species (e.g. Bushy) get a small "e" badge so players don't mistake them for a spawn that's broken.
